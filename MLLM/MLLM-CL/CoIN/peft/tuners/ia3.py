    ...
    def _check_target_module_exists(self, ia3_config, key):
        if isinstance(ia3_config.target_modules, str):
            ...
            target_module_found = any(
                self._is_valid_match(key, target_key)
                for target_key in ia3_config.target_modules
            )
        ...
        for key in key_list:
            ...
            if isinstance(target, IA3Layer):
                ...
                new_module = self._create_new_module(
                    ia3_config, adapter_name, target, is_feedforward
                )
...
class Linear(nn.Linear, IA3Layer):
    ...
    def merge(self):
        ...
        self.weight.data = torch.mul(
            self.weight.data, self.ia3_l[self.active_adapter].data
        )
        self.weight = transpose(self.weight, self.fan_in_fan_out)
    ...
    def unmerge(self):
        ...
        self.weight.data = torch.div(
            self.weight.data, self.ia3_l[self.active_adapter].data + 1e-8
        )
        self.weight = transpose(self.weight, self.fan_in_fan_out)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.is_feedforward:
                ...
                result = (
                    result.to(self.ia3_l[self.active_adapter].dtype)
                    * self.ia3_l[self.active_adapter].flatten()
                )
        else:
            result = F.linear(
                x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias
            ...
            if self.disable_adapters or self.active_adapter not in self.ia3_l.keys():
                ...
                if not torch.is_autocast_enabled():
                    ...
                    if self.is_feedforward:
                        ...
                        result = (
                            result * self.ia3_l[self.active_adapter].flatten()
                        ).to(expected_dtype)
                else:
                    if self.is_feedforward:
                        result = super().forward(
